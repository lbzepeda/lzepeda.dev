...
const TerminalLine: React.FC<{
  ...
  return (
    <div className={`font-mono ${getLineColor()}`}>
      {line}
      {isCurrentLine && (
        <span className="animate-pulse ml-0.5 text-[#383838] dark:text-[#C4C4C4]">
          ▊
        </span>
      )}
    </div>
...
export const Terminal: React.FC<TerminalProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [currentLine, currentChar, terminalLines, onComplete]);

  return (
    <div className="bg-[#FFFFFF] dark:bg-[#1E1E1E] font-mono p-4 rounded-lg shadow-lg w-full max-w-4xl mx-auto">
      <TerminalHeader />
